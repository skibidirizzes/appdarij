...
import { ClipboardListIcon, CheckCircleIcon, XCircleIcon, SpinnerIcon, FlashcardIcon } from './icons/index.ts';
import { Quiz, ScriptMode, Mistake, WordInfo, QuizQuestion } from '../types.ts';
import { QUIZ_LENGTH } from '../constants.ts';
import { useTranslations } from '../hooks/useTranslations.ts';

const MistakeItem: React.FC<{ mistake: Mistake }> = ({ mistake }) => {
    const { user, setWordToAddToDeck } = useContext(UserContext);
    const { scriptMode } = user.settings;

    const handleAddToDeck = () => {
        if (!mistake.question.targetWord) return;
        
        const wordInfo: WordInfo = {
            latin: mistake.question.targetWord.latin,
            arabic: mistake.question.targetWord.arabic,
            definition: mistake.question.explanation.latin, // Best guess for a definition
            examples: [{
                latin: mistake.question.targetWord.latin,
                arabic: mistake.question.targetWord.arabic,
                translation: mistake.question.question
            }]
        };
        setWordToAddToDeck(wordInfo);
    };

    return (
        <Card className="p-3 bg-slate-800/60">
            <p className="text-sm text-slate-400 mb-1">{mistake.question.question}</p>
            <div className="flex justify-between items-center">
                <p className="text-white">Your answer: <span className="font-semibold text-red-400">{String(mistake.userAnswer)}</span></p>
                {mistake.question.targetWord && (
                    <Button onClick={handleAddToDeck} size="sm" className="!p-2 bg-slate-600 hover:bg-slate-500">
...
const MistakesBankView: React.FC = () => {
    ...
    return (
        <div className="w-full animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                ...
                {/* Right Column */}
                <Card className="p-6 card-lift-hover h-full flex flex-col max-h-[600px]">
                    ...
                    {uniqueMistakes.length > 0 ? (
                        <div className="flex-1 space-y-3 overflow-y-auto pr-2">
                            {uniqueMistakes.map((mistake, index) => (
                                <MistakeItem key={`${mistake.question.id}-${index}`} mistake={mistake} />
                            ))}
                        </div>
                    ) : (
