...
const QuizInProgressToast: React.FC<QuizInProgressToastProps> = ({ onResume }) => {
    ...
    return (
        <div
            className="fixed top-5 right-5 w-80 bg-slate-800 border-2 border-amber-500 rounded-xl shadow-2xl p-4 flex items-center gap-4 z-[60] animate-fade-in-up"
            role="alert"
            aria-live="assertive"
        >
            <div className="text-2xl text-amber-400 flex-shrink-0"><ClockIcon /></div>
            <div>
                <h3 className="font-bold text-slate-100">Quiz in Progress</h3>
                <p className="text-slate-300 text-sm">Your quiz is paused. Don't forget to finish it to save your progress!</p>
